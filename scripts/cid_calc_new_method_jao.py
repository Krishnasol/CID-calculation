...
def get_jao_data(start_date:datetime, end_date:datetime, token):
    # Note: ToUtc time should be inclusive of the last hour (23:00), so we fetch through the end of the day
    url = f"https://publicationtool.jao.eu/nordic/api/data/fbDomainShadowPrice?Filter=%7B%7D&\
        Skip=0&Take=10000000&FromUtc={start_date}T00%3A00%3A00.000Z&ToUtc={end_date}T23%3A59%3A59.999Z"
    headers = {}
...
def create_dataframe_from_jao_data(start_date, end_date, token):
    ...
    num_days = delta.days
    # Fetch one day at a time as per new API limitation
    with alive_bar(num_days) as bar:
        time.sleep(0.005)
        for day in range(num_days):
            batch_date = start_date + timedelta(days=day)
            batch_date_str = batch_date.strftime('%Y-%m-%d')
            soup = get_jao_data(batch_date_str, batch_date_str, token)
            data = json.loads(soup)
            data_list = data.get("data", [])
            if data_list:  # Only append if there's data
                df = pd.DataFrame(data_list)
                dfs.append(df)
            print(f"Dataframe created for {batch_date_str}")
            bar()
...
def create_border_dataframe_from_jao_data(start_date, end_date, token):
    ...
    with alive_bar(num_days) as bar:
        ...
        for day in range(num_days):
            batch_date = start_date + timedelta(days=day)
            batch_date_str = batch_date.strftime('%Y-%m-%d')
            soup = get_jao_border_cnec_data(batch_date_str, batch_date_str, token)
            data = json.loads(soup)
            data_list = data.get("data", [])
            if data_list:  # Only append if there's data
                df = pd.DataFrame(data_list)
...
def calculate_CI_virtual_component(price_df, ptdf_df):
    ...
    ptdf_df_filtered['nordic_lambda'] = ptdf_df_filtered.index.map(no1_prices) + ptdf_df_filtered.index.map(nordic_lambda_values)
    ptdf_df_filtered['dk1_lambda'] = ptdf_df_filtered.index.map(dk1_prices) + ptdf_df_filtered.index.map(dk1_lambda_values)

    virtual_prices = pd.DataFrame(index=ptdf_df_filtered.index.unique(), columns=relevant_columns)
    ...
    for col in relevant_columns:
        if 'DK1' in col:
            ...
            virtual_prices[col] = ptdf_df_filtered['dk1_lambda'].groupby(ptdf_df_filtered.index).first() - summed_values
        else:
            summed_values = (
                ptdf_df_filtered.groupby(ptdf_df_filtered.index)
                .apply(lambda group: (group[col] * group['shadowPrice']).sum())
            )
            virtual_prices[col] = ptdf_df_filtered['nordic_lambda'].groupby(ptdf_df_filtered.index).first() - summed_values
...
if __name__ == "__main__":
    ...
    desired_order_virtual_prices = [
    "DK1_SB", "DK2_SB", "DK1_SK", "DK1_KS", "FI_FS",
    "NO2_SK", "SE3_FS", "SE3_KS", "SE3_SWL", "SE4_SWL",
    "DK1_CO", "DK1_DE", "DK2_KO", "FI_EL",
    "NO2_NK", "NO2_ND",
    "SE4_BC", "SE4_NB", "SE4_SP"]
    ...
    # jao_df_mesa = get_jao_mesa(start_date, end_date)
    # sp_df = get_shadow_price_mesa(start_date, end_date)
    # jao_df = merge_mesa_jao_and_sp(jao_df_mesa, sp_df)
    ...
    virtual_ci_component = virtual_ci_component.reindex(columns = desired_order_virtual_prices)
